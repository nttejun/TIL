...
### starter 프로젝트 개발을 진행했다.
+ AWS EC2 서버 연동을 완료했다.
+ EC2 서버로 로컬 JAR 파일 이동 시 SCP 이슈 발생
...
### starter 프로젝트 개발을 진행했다.
+ EC2 서버로 로컬 파일 이동 시 발생되는 이슈를 해결하고 [내용을 정리](http://wjjeong.tistory.com/14)했다.
+ EC2 서버에서 JAR 파일 실행실패 이슈를 해결하고 있다.
